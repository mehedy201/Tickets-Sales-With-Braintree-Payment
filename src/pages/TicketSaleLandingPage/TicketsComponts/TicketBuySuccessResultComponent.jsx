    ...
    return (
        <div>
            <div className="flex flex-col justify-center items-center py-8 gap-2">
                ...
                <p className="text-center">Transaction ID: {data?.data?.transactionID}</p>
            </div>
                ...
                </div>
                {
                    ...
                    <div className='flex justify-between py-2'>
                        <div>
                            ...
                            <span className='font-bold text-[12px] sm:text-[14px]'>{data?.data?.lowTicketsQuantity} x US$440.00</span>
                        </div>
                        <p className='font-bold text-[14px] sm:text-[16px]'>US$440.00</p>
                    </div>
                }
                {
                    ...
                    <div className='flex justify-between py-2'>
                        <div>
                            ...
                            <span className='font-bold text-[12px] sm:text-[14px]'>{data?.data?.fullTicketsQuantity} x US$500.00</span>
                        </div>
                        <p className='font-bold text-[14px] sm:text-[16px]'>US$500.00</p>
                    </div>
                }
                {
                    ...
                    <div className='flex justify-between py-2'>
                        <div>
                            ...
                            <span className='font-bold text-[12px] sm:text-[14px]'>{data?.data?.corporateTicketsQuantity} x US$550.00</span>
                        </div>
                        <p className='font-bold text-[14px] sm:text-[16px]'>US$550.00</p>
                    </div>
                }
                ...
                <div className='pt-4 border-t-1 border-gray-300'>
                    <div className='flex items-start justify-between'>
                        <div>
                            ...
                            <p className='text-sm'>{data?.data?.lowTicketsQuantity + data?.data?.fullTicketsQuantity + data?.data?.corporateTicketsQuantity} Tickets</p>
                        </div>
            ...
            <div className="p-5 max-w-[1000px] mx-auto">
                <h3 className="text-xl font-bold">Download Attendees Tickets</h3>
                {
                    ...
                    data?.data?.attendees.map((att, index) => 
                        <div key={index} className="py-3 shadow rounded-md flex justify-between border-b-1 border-gray-300">
                            <div>
                                <h3 className="text-xl font-semibold">{att?.firstName} {att?.lastName}</h3>
                                <p>{att?.ticketsType}</p>
                            </div>
                            <a style={{textDecoration: 'none'}} href={`http://localhost:5000/api/v1/icghc/download-attendees-tickets/${att._id}`}>Download Ticket</a>
                        </div>
                    )
                }
            </div>
